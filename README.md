...
The de novo assembly produced three contigs with lengths of approximately 3.3 Mb, 1.7 Mb, and 0.1 Mb, together accounting for the expected genome size of _Salmonella enterica_ [27]. The presence of only a small number of contigs indicates a high level of assembly contiguity, suggesting that long Oxford Nanopore reads effectively spanned repetitive regions that commonly fragment short-read assemblies. The two large contigs likely represent the chromosomal backbone, while the smaller circular contig is consistent with a plasmid. The large contig sizes and high sequencing coverage support the reliability of the assembly and indicate that the genome structure was largely reconstructed without extensive fragmentation.
...
Variant calling performed using BCFtools identified a total of 12,286 genetic variants relative to the _Salmonella enterica_ ASM694v2 reference genome, the majority of which were single nucleotide polymorphisms (Sn = 12,201), with a smaller number of insertions and deletions (n = 85) (Table 4). The strong predominance of SNPs over indels is consistent with expected strain-level divergence patterns and reflects the higher reliability of SNP detection in long-read sequencing data. The relatively low number of indels suggests effective read filtering and accurate alignment, as indel errors are a known challenge in Oxford Nanopore sequencing [28]. Overall, the observed variant profile supports the presence of genuine biological differences between the sequenced isolate and the laboratory reference genome and indicates that the alignment and variant calling pipeline successfully captured high-confidence genomic variation.
...
Figure 3. IGV visualizations of representative point mutations identified by BCFtools.
(A) IGV visualization of a synonymous point mutation corresponding to a T→C single-nucleotide substitution at position NC_003197.2:1,165,215 in the _Salmonella enterica_ LT2 reference genome. The variant is supported by multiple aligned reads and does not alter the encoded amino acid, with the codon continuing to encode valine (V), indicating a synonymous substitution.
(B) IGV visualization of a nonsynonymous point mutation corresponding to an A→G single-nucleotide substitution at position NC_003197.2:3,113,398. This substitution results in an amino acid change from isoleucine (I) to valine (V) and is supported by consistent read evidence across the alignment, indicating a true biological variant rather than a sequencing artifact.

# Discussion

High-quality genome assemblies are essential for accurately characterizing genetic variation and understanding how bacterial strains differ from reference lineages. In this study, we evaluated a computational pipeline for Oxford Nanopore–based genome assembly and polishing of _Salmonella enterica_, combined with reference-based comparative analyses to assess genome structure and sequence variation. The resulting assembly exhibited strong quality metrics, including high contiguity and a genome fraction of 95.7% as assessed by QUAST (Table 3). Comparative alignment to the _Salmonella enterica_ serovar Typhimurium LT2 reference genome (NCBI assembly ASM694v2) revealed overall structural concordance, while variant analysis demonstrated that sequence divergence was dominated by single-nucleotide polymorphisms with a low indel burden (Table 4). Together, these results highlight the ability of modern long-read sequencing and bioinformatics tools to enable rapid, high-quality bacterial genome assembly and comparative analysis, supporting investigations into bacterial evolution, virulence, and host–pathogen interactions [28].

The Flye assembly recovered the majority of the reference genome across three contigs, consistent with a bacterial chromosome and a plasmid [28]. QUAST identified a limited number of misassemblies classified as relocations, which have been reported in long-read assemblies and are often associated with unresolved repetitive regions such as ribosomal operons or mobile genetic elements [29]. These structural discrepancies may also reflect genuine biological differences between the sequenced isolate and the laboratory reference strain, as chromosomal rearrangements are known to occur among _Salmonella enterica_ isolates [30]. Despite these relocations, overall genome fraction, GC content, and contiguity metrics remained high, indicating that the assembly preserved the global genome structure and captured the majority of the reference sequence.

Polishing the Flye assembly with Medaka resulted in modest improvements in base-level accuracy, as evidenced by reduced mismatch and indel rates per 100 kbp in the polished assembly. Assembly-wide metrics such as contig number, N50, and genome fraction remained unchanged, indicating that polishing primarily corrected residual base-calling errors rather than altering large-scale genome structure. This outcome is consistent with expectations for high-coverage Oxford Nanopore R10.4 datasets, where Flye produces a high-quality consensus assembly and additional polishing yields diminishing returns [31]. The stability of assembly metrics before and after polishing supports the reliability of the final consensus sequence for downstream comparative analyses.

Variant calling using BCFtools identified 12,286 variants relative to the reference genome, including 12,201 single nucleotide polymorphisms (SNPs) and 85 small insertions or deletions. The predominance of SNPs relative to indels likely reflects both limited sequence divergence between the isolate and the reference genome and the conservative behavior of pileup-based variant calling. The low number of indels suggests reliable read alignment and filtering, minimizing the inclusion of false-positive indel calls, which are more common in long-read sequencing data [28].

Visualization of selected variants in IGV confirmed strong read support, high mapping quality, and consistent allele frequencies at variant sites. One observed T→C substitution represents a synonymous point mutation, as the altered codon encodes the same valine residue according to the IGV translation track. Although synonymous mutations do not alter the amino acid sequence, they may still influence gene expression through codon usage bias or mRNA structure. A second variant involved an A→G substitution that resulted in a nonsynonymous point mutation, producing an amino acid change from isoleucine to valine. While this substitution is chemically conservative, even single-residue changes can influence protein stability or function depending on their genomic context. Together, these variants illustrate the coexistence of neutral and potentially functional point mutations within the genome. Variant calling using bcftools identified 12,201 single-nucleotide polymorphisms, corresponding to point mutations, and 85 short indels representing insertions or deletions. The low number of indels relative to SNPs suggests an absence of a substantial frameshift burden, indicating that most sequence variation occurs as single-base substitutions rather than disruptive length-altering mutations.

This study highlights several strengths of the applied computational pipeline. Long-read Oxford Nanopore sequencing enabled the generation of a highly contiguous genome assembly and facilitated the resolution of genomic regions that are challenging for short-read approaches [32]. Integration of reference-based assembly evaluation, variant calling, and visualization in IGV strengthened confidence in both structural accuracy and variant calls. However, there are several limitations thatshould be acknowledged. Analyses were conducted relative to a single reference genome (NCBI Assembly ASM694v2), which may limit detection of more divergent or strain-specific features. Despite polishing, residual sequencing errors may persist, particularly in low-complexity or methylated regions [4]. Additionally, the absence of short-read validation limits direct assessment of base-level accuracy. Future work could address these limitations through hybrid assembly approaches, functional annotation of coding-region variants, and phylogenetic analysis using shared SNPs across multiple _Salmonella_ isolates. Overall, this workflow demonstrates the effectiveness of long-read sequencing–based genome assembly and comparative analysis for studying bacterial genome structure and variation.
...
[27] Stevens, M. J. A., Zurfluh, K., Althaus, D., Corti, S., Lehner, A., & Stephan, R. (2018). Complete and Assembled Genome Sequence of Salmonella enterica subsp. enterica Serotype Senftenberg N17-509, a Strain Lacking Salmonella Pathogen Island 1. Genome Announcements (Washington, DC), 6(12), Article 00156. https://doi.org/10.1128/genomeA.00156-18

[28] Wick, R. R., Judd, L. M., & Holt, K. E. (2023). Assembling the perfect bacterial genome using Oxford Nanopore and Illumina sequencing. PLoS Computational Biology, 19(3), Article 1010905. https://doi.org/10.1371/journal.pcbi.1010905

[29] Acuña-Amador, L., Primot, A., Cadieu, E., Roulet, A., & Barloy-Hubler, F. (2018). Genomic repeats, misassembly and reannotation: a case study with long-read resequencing of Porphyromonas gingivalis reference strains. BMC Genomics, 19(1), Article 54. https://doi.org/10.1186/s12864-017-4429-4

[30] Helm, R. A., Lee, A. G., Christman, H. D., & Maloy, S. (2003). Genomic Rearrangements at rrn Operons in Salmonella. Genetics (Austin), 165(3), 951–959. https://doi.org/10.1093/genetics/165.3.951
...
[32] Zhao, W., Zeng, W., Pang, B., Luo, M., Peng, Y., Xu, J., Kan, B., Li, Z., & Lu, X. (2023). Oxford nanopore long-read sequencing enables the generation of complete bacterial and plasmid genomes without short-read sequencing. Frontiers in Microbiology, 14, Article 1179966. https://doi.org/10.3389/fmicb.2023.1179966
